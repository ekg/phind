#!/usr/bin/env python3
"""Independent checksum and semantic validator for canonical-cohort-250-v1."""
...
def validate(repo: Path, external: Path, samtools: str = "samtools", bgzip: str = "bgzip") -> dict[str, Any]:
    ...
    check(release.get("input_cohort_0250_sha256") == r.COHORT_SHA256, "cohort SHA mismatch")
    ...
    for name, verdict in release.get("applicable_gates", {}).items():
        ...
        elif name == "integrated_n100_go_250":
            check(verdict == "NOT_APPLICABLE_CANONICAL_PREPARATION_ONLY", "integrated gate applicability mismatch")
        else:
            check(verdict == "PASS", f"release gate is not PASS: {name}={verdict}")
    groups.append("release_identity_and_all_applicable_gates")

    cohort_external = external / "manifests/cohort-0250.tsv"
    cohort_tracked = repo / "manifests/pilot-cohorts-v1/cohort-0250.tsv"
    ...
    check(len(batches) == 25, "bounded batch row cardinality mismatch")
    expected_manifest_rows = {
        "cohort-0250.tsv": r.COHORT_ROWS,
    ...
    for order, (assembly, ref) in enumerate(zip(assemblies, refs), 1):
        ...
        if order <= r.PREDECESSOR_ROWS:
            ...
            check(old is not None and all(assembly[field] == old[field] for field in reuse_digest_fields), f"N=100 checksum reuse mismatch: {accession}")
        else:
    ...
    groups.append("all_250_source_bgzf_index_crosswalk_annotation_semantics")
    ...
    check(request_accessions.issubset(set(accessions[r.PREDECESSOR_ROWS:])), "download outside frozen new 150 rows")
    ...
    check(all(value == 0 for value in partial_stage_bytes[10:]), "partial stage survived its deterministic resume batch")
    ...
    check(cap_evidence.get("start", {}).get("distinct_exact_assembly_revisions") == r.PREDECESSOR_ROWS, "start union was not exact N=100")
    ...
    check(global_finish["distinct_exact_assembly_revisions"] == r.COHORT_ROWS, "finish union cardinality is not 250")
    ...
    owned_roots = [
        repo / "manifests/canonical-cohort-250-v1",
        repo / "artifacts/canonical_cohort_250",
        repo / "reports/canonical_cohort_250.md",
    ]
    owned_files = [
    ...
    return {
        "schema": "canonical-cohort-250-validation-v1", "verdict": "PASS",
