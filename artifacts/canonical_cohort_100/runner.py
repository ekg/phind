#!/usr/bin/env python3
"""Fail-closed N=100 canonical cohort preparation.

This task-owned driver deliberately imports the frozen Stage-B implementation
rather than changing it.  The first ten objects are immutable digest references
to canonical-cohort-010-v1; only cohort rows 11..100 can reach the downloader.
"""
...
TASK_ID = "prepare-canonical-cohort-100"
SCHEMA = "canonical-cohort-100-release-v1"
POLICY = p.POLICY
...
COHORT_SHA256 = "13e203961a9fcec18a8a09e690582652d8085b2a386811e6c6a03184b9489182"
COHORT_BYTES = 36573
COHORT_ROWS = 100
...
DURABLE_PREFIX = "/home/erikg/phind-data/ecoli26k/v1/releases/prepare-canonical-cohort-100/"
SCRATCH_PREFIX = "/mnt/nvme3n1/erikg/phind-genome-work/ecoli26k/v1/prepare-canonical-cohort-100/"
...
BATCH_FIELDS = [
    ...
    "stage_bytes_start", "stage_bytes_finish", "stage_files_finish", "row_sha256",
]
...
def configure_pinned_primitives(accessions: list[str]) -> None:
    """Narrow the inherited URL constructor to the exact frozen 100 rows."""
    p.EXPECTED_ACCESSIONS = list(accessions)
    p.TASK_ID = TASK_ID
    p.USER_AGENT = "phind-canonical-cohort-100/1.0 (bounded-frozen-100)"
...
def verify_inputs(repo: Path) -> dict[str, Any]:
    ...
    if selection_sums.get("cohort-0100.tsv") != COHORT_SHA256:
        raise GateError("N=100 selection manifest inventory mismatch")
    ...
    cohort = selection_dir / "cohort-0100.tsv"
    if cohort.stat().st_size != COHORT_BYTES or sha_file(cohort) != COHORT_SHA256:
        raise GateError("frozen N=100 manifest bytes/SHA mismatch")
    external_cohort = sel_external / "manifests/cohort-0100.tsv"
    if not external_cohort.is_file():
        external_cohort = sel_external / "cohort-0100.tsv"
    if external_cohort.read_bytes() != cohort.read_bytes():
        raise GateError("tracked/external N=100 manifest bytes differ")
    ...
    if len(rows) != COHORT_ROWS or len(set(accessions)) != COHORT_ROWS:
        raise GateError("N=100 manifest cardinality/duplicate gate failed")
    for order, row in enumerate(rows, 1):
        if int(row["cohort_order"]) != order or row["rung_n"] != "100":
            raise GateError("N=100 cohort order/rung gate failed")
        if not p.ACCESSION_RE.fullmatch(row["exact_assembly_accession_version"]):
            raise GateError("N=100 manifest contains invalid exact accession")
        if row["resolution_status"] not in ("EXACT_VERSION_RESOLVED", "EXACT_VERSION_VALID_METADATA_UNAVAILABLE"):
            raise GateError("N=100 manifest contains non-exact/non-validated row")

    predecessor_dir = repo / "manifests/canonical-cohort-010-v1"
    ...
    if predecessor_sums.get("release.json") != PREDECESSOR_RELEASE_JSON_SHA256:
        raise GateError("N=10 predecessor release.json pinned SHA-256 mismatch")
    ...
    if (predecessor.get("release_id") != PREDECESSOR_RELEASE_ID or predecessor.get("verdict") != "PASS"
        ...
        raise GateError("N=10 predecessor identity/verdict/immutability mismatch")
    _require_pass_or_na(predecessor.get("applicable_gates", {}), "N=10 predecessor")
    ...
    if sha_file(predecessor_external / "release.json") != PREDECESSOR_RELEASE_JSON_SHA256:
        raise GateError("N=10 tracked/external release.json mismatch")
    predecessor_accessions = predecessor.get("sequence_bearing_assembly_accessions", [])
    if accessions[:10] != predecessor_accessions or len(predecessor_accessions) != 10:
        raise GateError("N=100 is not exact nested extension of N=10")
...
def release_seed(accessions: list[str]) -> dict[str, Any]:
    return {
        ...
        "cohort_0100_sha256": COHORT_SHA256,
...
def release_id(accessions: list[str]) -> str:
    return "canonical-cohort-100-v1-" + p.sha_bytes(canonical_json(release_seed(accessions)))[:16]
...
def live_preflight(durable: Path, scratch: Path, allocations: p.Allocations, stage_name: str) -> dict[str, Any]:
    ...
    return {
        "schema": "canonical-100-resource-preflight-v1", "verdict": "PASS", "stage": stage_name,
...
def _storage_for(order: int, stage: Path, predecessor_external: Path) -> tuple[Path, str, str, str]:
    if order <= 10:
        return predecessor_external, PREDECESSOR_RELEASE_ID, str(predecessor_external), "REUSED_PREDECESSOR_BY_DIGEST"
...
def build_release_tables(stage: Path, inputs: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for input_row in inputs["rows"]:
        ...
        accession = input_row["exact_assembly_accession_version"]
        root, storage_id, storage_root, reuse = _storage_for(order, stage, inputs["predecessor_external"])
        ...
        if order <= 10:
            ...
            if old is None or any(str(assembly[field]) != old[field] for field in digest_fields):
                raise GateError(f"reused N=10 checksum contract mismatch: {accession}")
        assembly_rows.append(assembly)
        object_contigs = p.read_tsv(canonical_dir / "contigs.tsv", p.CONTIG_FIELDS, verify_hashes=True)
        if order > 10 and any(int(row["stage_b_order"]) != order for row in object_contigs):
        ...
        refs.append({
            ...
            "reuse_status": reuse, "predecessor_digest_match": "PASS" if order <= 10 else "NOT_APPLICABLE_NEW_OBJECT",
        })
        roles = (
        ...
        state_rows.append({"stage_b_order": order, "accession": accession, "source_state": "COMPLETE",
                           ...
                           "reason": "REUSED_PREDECESSOR_BY_DIGEST" if order <= 10 else "."})
...
def _write_tables(stage: Path, inputs: dict[str, Any], batch_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if len(assemblies) != 100 or len(states) != 100 or len(refs) != 100 or len(checksums) != 800:
...
def publish_tracked(external: Path, tracked: Path, artifact: Path) -> None:
    ...
    mapping = {
        "manifests/cohort-0100.tsv": "cohort-0100.tsv",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(projected) != 100 or projected != set(inputs["accessions"]) or not projected.issubset(allowed):
        raise GateError("projected global union is not exactly frozen N=100")
    ...
    shutil.copyfile(repo / "manifests/pilot-cohorts-v1/cohort-0100.tsv", stage / "manifests/cohort-0100.tsv")
    ...
    provenance = {
        "schema": "canonical-cohort-100-provenance-v1", "task_id": TASK_ID, "release_id": rid,
        ...
        "max_task_distinct_exact_assembly_revisions": 100, "global_cap": 1000,
        ...
        "reuse_policy": "rows 1-10 are read-only predecessor digest references; no copy, download, or recompression",
    ...
    global_evidence = {"schema": "global-distinct-assembly-cap-v1", "verdict": "PASS", "start": global_start,
        ...
        "projected_finish_accessions": sorted(projected), "task_bound": 100, "graph_cap": 1000,
    ...
    try:
        ...
        for offset in range(0, len(rows), args.batch_size):
            ...
            for input_row in batch:
                ...
                accession = input_row["exact_assembly_accession_version"]
                if order <= 10:
                    _read_object(inputs["predecessor_external"], accession)
                    already += 1
                    p.append_jsonl(state, {"event": "PREDECESSOR_OBJECT_REUSED_BY_DIGEST", "cohort_order": order,
                                           "accession": accession, "release_id": PREDECESSOR_RELEASE_ID, "at": p.utcnow()})
                    continue
            ...
            batch_rows.append({"batch_number": number, "first_cohort_order": int(batch[0]["cohort_order"]),
                ...
                "stage_bytes_start": before[0], "stage_bytes_finish": finish_usage[0], "stage_files_finish": finish_usage[1]})
        ...
        peak_rss = ru_end.ru_maxrss * 1024
        ...
        resource_summary = {
            "schema": "canonical-cohort-100-resource-summary-v1", "verdict": "PASS",
        ...
        restart = {
            "schema": "canonical-cohort-100-restart-evidence-v1",
            ...
            "predecessor_objects_never_downloaded_or_recompressed": all(f'"accession":"{accession}"' not in "\n".join(line for line in state_text.splitlines() if "ACQUISITION_REQUEST_STARTED" in line or "CANONICAL_OBJECT_VALIDATED" in line) for accession in inputs["accessions"][:10]),
        }
        ...
        release = {
            ...
            "input_cohort_0100_sha256": COHORT_SHA256, "predecessor_release_id": PREDECESSOR_RELEASE_ID,
            "predecessor_release_json_sha256": PREDECESSOR_RELEASE_JSON_SHA256,
            "compatibility_release_id": COMPATIBILITY_RELEASE_ID,
            "compatibility_release_json_sha256": COMPATIBILITY_RELEASE_JSON_SHA256,
            ...
            "counts": {"attempted_exact_assembly_revisions": 100, "validated": 100, "quarantined": 0,
                "reused_predecessor_objects": 10, "new_objects": 90,
                "distinct_sequence_bearing_assemblies": 100, "global_distinct_assembly_cap": 1000,
            ...
            "object_storage_contract": "rows 1-10 resolve read-only by predecessor release ID+object inventory SHA; rows 11-100 are self-contained",
        ...
        manifest_specs = {
            "cohort-0100.tsv": 100, "assemblies.tsv": 100, "contigs.tsv": len(contigs),
            "checksums.tsv": 800, "state.tsv": 100, "object_refs.tsv": 100, "batch_metrics.tsv": len(batch_rows),
        }
        ...
        if global_finish["distinct_exact_assembly_revisions"] != 100 or set(global_finish["accessions"]) != set(inputs["accessions"]):
            raise GateError("finish global union is not exactly frozen N=100")
        publish_tracked(final, tracked, artifact)
        return release
...
def parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tracked-root", default="manifests/canonical-cohort-100-v1")
    parser.add_argument("--artifact-root", default="artifacts/canonical_cohort_100")
...
def main() -> int:
    try:
        ...
        if args.inject_accession is None:
            args.inject_accession = inputs["accessions"][10]
        if args.inject_accession not in inputs["accessions"][10:]:
            raise GateError("kill injection must target a new frozen cohort row 11..100")
