#!/usr/bin/env python3
"""Independent checksum and semantic validator for canonical-cohort-500-v1."""
...
from artifacts.canonical_cohort_500 import runner as r
...
def validate(repo: Path, external: Path, samtools: str = "samtools", bgzip: str = "bgzip") -> dict[str, Any]:
    ...
    check(release.get("input_cohort_0500_sha256") == r.COHORT_SHA256, "cohort SHA mismatch")
    ...
    check(release.get("canonical_go_500_verdict") == "CANONICAL_GO_500", "CANONICAL_GO_500 verdict missing")
    check(release.get("canonical_go_500_verdict_sha256") == r.AUDIT_VERDICT_SHA256, "CANONICAL_GO_500 checksum mismatch")
    check(release.get("canonical_n250_audit_sha256") == r.AUDIT_RESULT_SHA256, "canonical N=250 audit checksum mismatch")
    ...
    check(gates.get("canonical_go_500") == "PASS", "canonical GO gate missing")
    check(gates.get("canonical_scale_trend") == "PASS", "canonical scale gate missing")
    ...
    tracked = repo / "manifests/canonical-cohort-500-v1"
    ...
    tracked_mapping = {
        "cohort-0500.tsv": "manifests/cohort-0500.tsv",
    ...
    cohort_external = external / "manifests/cohort-0500.tsv"
    cohort_tracked = repo / "manifests/pilot-cohorts-v1/cohort-0500.tsv"
    ...
    check(len(batches) == 50, "bounded batch row cardinality mismatch")
    expected_manifest_rows = {
        "cohort-0500.tsv": r.COHORT_ROWS,
    ...
    for order, (assembly, ref) in enumerate(zip(assemblies, refs), 1):
        ...
        if order <= r.PREDECESSOR_ROWS:
            ...
            check(old is not None and all(assembly[field] == old[field] for field in reuse_digest_fields), f"N=250 checksum reuse mismatch: {accession}")
        else:
    ...
    groups.append("all_500_source_bgzf_index_crosswalk_annotation_semantics")
    ...
    check(request_accessions.issubset(set(accessions[r.PREDECESSOR_ROWS:])), "download outside frozen new 250 rows")
    ...
    first_swap_free = int(resources[0]["swap_free_bytes"])
    check(min(int(row["swap_free_bytes"]) for row in resources) >= first_swap_free, "sampled task-lifetime swap growth detected")
    ...
    recomputed_scale = r.compute_scale_trend(
        repo, inputs, assemblies, float(summary["invocation_wall_seconds"]), int(summary["peak_rss_bytes"]),
        int(summary["measured_release_stage_peak_bytes"]), int(summary["measured_release_stage_peak_files"]),
        p.Allocations(**summary["allocations"]), float(scale["n1000_projection"]["time_allocation_seconds"]),
    )
    check(recomputed_scale == scale, "canonical scale trend/projection recomputation mismatch")
    ...
    check(all(scale["n1000_projection"]["checks"].values()), "N=1,000 upper-95 projection failure")
    ...
    check(cap_evidence.get("start", {}).get("distinct_exact_assembly_revisions") == r.PREDECESSOR_ROWS, "start union was not exact N=250")
    ...
    check(global_finish["distinct_exact_assembly_revisions"] == r.COHORT_ROWS, "finish union cardinality is not 500")
    ...
    owned_roots = [
        repo / "manifests/canonical-cohort-500-v1",
        repo / "artifacts/canonical_cohort_500",
        repo / "reports/canonical_cohort_500.md",
    ]
    owned_files = [
    ...
    return {
        "schema": "canonical-cohort-500-validation-v1", "verdict": "PASS",
