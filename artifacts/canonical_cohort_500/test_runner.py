...
class CanonicalCohort500Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_pinned_real_inputs_are_exact_nested_500_and_all_required_gates_pass(self):
        ...
        predecessor_root, predecessor_id, _, _ = r._storage_for(250, inputs["accessions"][249], Path("stage"), inputs)
        ...
        self.assertEqual(inputs["canonical_go_500"]["verdict"], "CANONICAL_GO_500")
        self.assertEqual(inputs["canonical_audit"]["audit_verdict"], "PASS")

    def test_manifest_row_checksum_mismatch_fails_closed(self):
        original = Path("manifests/pilot-cohorts-v1/cohort-0500.tsv").read_text()
    ...
    def test_url_constructor_is_bounded_to_exact_frozen_500(self):
        inputs = r.verify_inputs(Path(".").resolve())
        r.configure_pinned_primitives(inputs["accessions"])
        self.assertIn(inputs["accessions"][499], p.download_url(inputs["accessions"][499]))
    ...
    def test_canonical_go_500_checksum_mismatch_fails_closed(self):
        with mock.patch.object(r, "verify_tracked_inventory", return_value={"verdict.json": "0" * 64}):
            with self.assertRaisesRegex(p.GateError, "CANONICAL_GO_500 verdict mismatch"):
                r._verify_canonical_go_500(Path("."), Path("unused"))
    ...
    def test_release_identity_is_order_and_pin_deterministic(self):
        ...
        self.assertTrue(first.startswith("canonical-cohort-500-v1-"))
