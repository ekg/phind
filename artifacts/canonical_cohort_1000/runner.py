#!/usr/bin/env python3
"""Fail-closed N=1,000 canonical cohort preparation.

This task-owned driver deliberately imports the frozen Stage-B implementation
rather than changing it. The first 500 objects are immutable digest references
to canonical-cohort-500-v1; only cohort rows 501..1,000 can reach the downloader.
No projection or processing beyond the frozen 1,000-assembly ceiling exists.
"""
...
TASK_ID = "prepare-canonical-cohort-1000"
SCHEMA = "canonical-cohort-1000-release-v1"
POLICY = p.POLICY
...
COHORT_BYTES = 365970
COHORT_ROWS = 1000
PREDECESSOR_ROWS = 500
NEW_ROWS = COHORT_ROWS - PREDECESSOR_ROWS
PREDECESSOR_RELEASE_ID = "canonical-cohort-500-v1-c208c111708b6435"
PREDECESSOR_RELEASE_JSON_SHA256 = "1df34fa84f2d4223242cad20529bea331c67289653f082a8f6da4d5036764dd5"
PREDECESSOR_SCALE_TREND_SHA256 = "11edf88a6a9b5b126917cb4d734ed683aeb01a56647d0631067e4162c84ae686"
PREDECESSOR_ARTIFACT_INVENTORY_SHA256 = "d7108a03443150863220b287f72104ebd3f1f32612ee3325ec9e730e1e0b05d4"
PREDECESSOR_EXTERNAL_SHA256SUMS_SHA256 = "214cf5b1885004f1faa5ddda5e9423c73f78c2cc85e6c36aaca23c0151bc3b18"
PREDECESSOR_COMPLETE_SHA256 = "e688d951618e42566edf7906de643bfe0927739484ad33ac96626321da32d7b1"
PREDECESSOR_TREE_SHA256 = "711e2720d2890dd391a74f8c757b389bf9277f0202bff3ed8682a2d328a96936"
...
DURABLE_PREFIX = "/home/erikg/phind-data/ecoli26k/v1/releases/prepare-canonical-cohort-1000/"
SCRATCH_PREFIX = "/mnt/nvme3n1/erikg/phind-genome-work/ecoli26k/v1/prepare-canonical-cohort-1000/"
...
def configure_pinned_primitives(accessions: list[str]) -> None:
    """Narrow the inherited URL constructor to the exact frozen 1,000 rows."""
    p.EXPECTED_ACCESSIONS = list(accessions)
    p.TASK_ID = TASK_ID
    p.USER_AGENT = "phind-canonical-cohort-1000/1.0 (bounded-frozen-1000)"
...
def _verify_canonical_scale_projection(
    repo: Path, predecessor_external: Path, predecessor: dict[str, Any]
) -> dict[str, Any]:
    """Consume the checksum-pinned N=1,000 projection emitted by canonical N=500."""
    artifact_dir = repo / "artifacts/canonical_cohort_500"
    if sha_file(artifact_dir / "SHA256SUMS") != PREDECESSOR_ARTIFACT_INVENTORY_SHA256:
        raise GateError("canonical N=500 artifact inventory checksum mismatch")
    artifact_inventory = verify_tracked_inventory(artifact_dir)
    if artifact_inventory.get("scale_trend.json") != PREDECESSOR_SCALE_TREND_SHA256:
        raise GateError("pinned canonical N=500 scale_trend.json inventory mismatch")
    validation_digest = artifact_inventory.get("validation.json")
    if not validation_digest:
        raise GateError("canonical N=500 pinned validation result missing")
    validation = json.loads((artifact_dir / "validation.json").read_text())
    if (validation.get("verdict") != "PASS"
            or validation.get("release_id") != PREDECESSOR_RELEASE_ID
            or validation.get("assembly_rows") != PREDECESSOR_ROWS
            or validation.get("global_distinct_exact_assembly_revisions") != PREDECESSOR_ROWS):
        raise GateError("canonical N=500 validation result is not unqualified exact-N=500 PASS")
    tracked_scale = artifact_dir / "scale_trend.json"
    external_scale = predecessor_external / "scale_trend.json"
    if sha_file(external_scale) != PREDECESSOR_SCALE_TREND_SHA256:
        raise GateError("scale_trend.json checksum mismatch")
    if tracked_scale.read_bytes() != external_scale.read_bytes():
        raise GateError("tracked/external canonical N=500 scale_trend.json bytes differ")
    if predecessor.get("applicable_gates", {}).get("canonical_scale_trend") != "PASS":
        raise GateError("canonical N=500 applicable_gates.canonical_scale_trend is not PASS")
    if predecessor.get("scale_trend_sha256") != PREDECESSOR_SCALE_TREND_SHA256:
        raise GateError("canonical N=500 release scale trend pin mismatch")
    if (sha_file(predecessor_external / "SHA256SUMS") != PREDECESSOR_EXTERNAL_SHA256SUMS_SHA256
            or sha_file(predecessor_external / "COMPLETE") != PREDECESSOR_COMPLETE_SHA256
            or _tree_digest(predecessor_external) != PREDECESSOR_TREE_SHA256):
        raise GateError("live canonical N=500 immutable inventory/tree pin mismatch")
    scale = json.loads(external_scale.read_text())
    projection = scale.get("n1000_projection", {})
    if scale.get("verdict") != "PASS" or not scale.get("checks") or not all(scale["checks"].values()):
        raise GateError("canonical N=500 scale_trend.json verdict/checks are not unqualified PASS")
    if (projection.get("target_rung") != 1000
            or projection.get("incremental_objects") != 500
            or not projection.get("checks")
            or not all(projection["checks"].values())):
        raise GateError("canonical N=1,000 projection checks are not unqualified PASS")
    return scale
...
def verify_inputs(repo: Path) -> dict[str, Any]:
    ...
    if selection_sums.get("cohort-1000.tsv") != COHORT_SHA256:
        raise GateError("N=1,000 selection manifest inventory mismatch")
    ...
    cohort = selection_dir / "cohort-1000.tsv"
    if cohort.stat().st_size != COHORT_BYTES or sha_file(cohort) != COHORT_SHA256:
        raise GateError("frozen N=1,000 manifest bytes/SHA mismatch")
    external_cohort = sel_external / "manifests/cohort-1000.tsv"
    if not external_cohort.is_file():
        external_cohort = sel_external / "cohort-1000.tsv"
    if external_cohort.read_bytes() != cohort.read_bytes():
        raise GateError("tracked/external N=1,000 manifest bytes differ")
    ...
    if len(rows) != COHORT_ROWS or len(set(accessions)) != COHORT_ROWS:
        raise GateError("N=1,000 manifest cardinality/duplicate gate failed")
    for order, row in enumerate(rows, 1):
        if int(row["cohort_order"]) != order or row["rung_n"] != "1000":
            raise GateError("N=1,000 cohort order/rung gate failed")
        if not p.ACCESSION_RE.fullmatch(row["exact_assembly_accession_version"]):
            raise GateError("N=1,000 manifest contains invalid exact accession")
        if row["resolution_status"] not in ("EXACT_VERSION_RESOLVED", "EXACT_VERSION_VALID_METADATA_UNAVAILABLE"):
            raise GateError("N=1,000 manifest contains non-exact/non-validated row")

    predecessor_dir = repo / "manifests/canonical-cohort-500-v1"
    ...
    if predecessor_sums.get("release.json") != PREDECESSOR_RELEASE_JSON_SHA256:
        raise GateError("N=500 predecessor release.json pinned SHA-256 mismatch")
    ...
    if (predecessor.get("release_id") != PREDECESSOR_RELEASE_ID or predecessor.get("verdict") != "PASS"
        ...
        raise GateError("N=500 predecessor identity/verdict/immutability mismatch")
    _require_pass_or_na(predecessor.get("applicable_gates", {}), "N=500 predecessor")
    ...
    if sha_file(predecessor_external / "release.json") != PREDECESSOR_RELEASE_JSON_SHA256:
        raise GateError("N=500 tracked/external release.json mismatch")
    canonical_scale_trend = _verify_canonical_scale_projection(repo, predecessor_external, predecessor)
    ...
    if accessions[:PREDECESSOR_ROWS] != predecessor_accessions or len(predecessor_accessions) != PREDECESSOR_ROWS:
        raise GateError("N=1,000 is not exact nested extension of N=500")
    ...
    if (len(predecessor_refs) != PREDECESSOR_ROWS
        ...
        raise GateError("N=500 predecessor object reference accounting/order mismatch")
    ...
    return {
        ...
        "canonical_scale_trend": canonical_scale_trend,
...
def release_id(accessions: list[str]) -> str:
    return "canonical-cohort-1000-v1-" + p.sha_bytes(canonical_json(release_seed(accessions)))[:16]
...
def live_preflight(durable: Path, scratch: Path, allocations: p.Allocations, stage_name: str) -> dict[str, Any]:
    ...
    return {
        "schema": "canonical-1000-resource-preflight-v1", "verdict": "PASS", "stage": stage_name,
    ...
    for input_row in inputs["rows"]:
        ...
        if order <= PREDECESSOR_ROWS:
            ...
            if old is None or any(str(assembly[field]) != old[field] for field in digest_fields):
                raise GateError(f"reused N=500 checksum contract mismatch: {accession}")
...
def publish_tracked(external: Path, tracked: Path, artifact: Path) -> None:
    ...
    mapping = {
        "manifests/cohort-1000.tsv": "cohort-1000.tsv",
...
def compute_scale_trend(
    ...
    """Finalize canonical scaling at N=1,000 without projecting or authorizing N>1,000."""
    predecessor_scale = inputs["canonical_scale_trend"]
    observations = list(predecessor_scale["rung_observations"])
    if [int(row["rung"]) for row in observations] != [10, 100, 250, 500]:
        raise GateError("pinned canonical resource history rung identity/order mismatch")
    predecessor_projection = predecessor_scale["n1000_projection"]
    if float(predecessor_projection["time_allocation_seconds"]) != n1000_time_allocation_seconds:
        raise GateError("N=1,000 explicit time allocation differs from pinned projection")
    ...
    if any(float(current[name]) <= 0 for name in (
        ...
        raise GateError("blank/non-positive N=1,000 resource observation")
    if current["incremental_objects"] != NEW_ROWS:
        raise GateError("N=1,000 incremental resource row count mismatch")
    ...
    prior_upper_bound = float(predecessor_scale["time_exponent"]["empirical_upper_bound"])
    exponent_upper_bound = max(prior_upper_bound, current_exponent)
    ...
    slope_changes = {name: current_slopes[name] / prior_slopes[name] - 1.0 for name in current_slopes}
    checks = _enforce_scale_thresholds(exponent_upper_bound, slope_changes)
    projection_checks = {
        "measured_wall_within_pinned_n1000_upper95": current["wall_seconds"] <= predecessor_projection["wall_upper95_seconds"],
        "measured_source_within_pinned_n1000_upper95": current["source_bytes"] <= predecessor_projection["source_transfer_upper95_bytes"],
        "measured_stage_within_pinned_n1000_upper95": current["stage_bytes"] <= predecessor_projection["stage_upper95_bytes"],
        "measured_files_within_pinned_n1000_upper95": current["stage_files"] <= predecessor_projection["files_upper95"],
        "measured_rss_within_pinned_n1000_upper95": current["peak_rss_bytes"] <= predecessor_projection["rss_upper95_bytes"],
        "measured_wall_within_explicit_time_allocation": current["wall_seconds"] <= n1000_time_allocation_seconds,
        "measured_rss_le_70pct_assigned_ram": current["peak_rss_bytes"] * 100 <= allocations.assigned_ram_bytes * 70,
        "measured_disk_le_70pct_allocation": current["stage_bytes"] * 100 <= allocations.durable_allocation_bytes * 70,
        "measured_files_le_50pct_inode_allocation": current["stage_files"] * 2 <= allocations.inode_allocation,
        "allocation_retains_2x_unfinished_after_measured_peak": allocations.durable_allocation_bytes - current["stage_bytes"] >= 2 * allocations.unfinished_write_bytes,
    }
    if not all(projection_checks.values()):
        raise GateError("N=1,000 measured-versus-pinned projection NO_GO: " + json.dumps(projection_checks, sort_keys=True))
    checks.update(projection_checks)
    all_observations = observations + [current]
    models = {name: _power_fit_predict(all_observations, name, int(current["incremental_bases"])) for name in slope_metrics}
    return {
        "schema": "canonical-cohort-1000-final-scale-trend-v1",
        "verdict": "PASS",
        "authorized_ceiling": 1000,
        "projection_beyond_ceiling": "NOT_AUTHORIZED_NOT_COMPUTED",
        "predecessor_scale_trend_sha256": PREDECESSOR_SCALE_TREND_SHA256,
        "time_exponent": {
            "prior_upper_bound_through_n500": prior_upper_bound,
            "current_n500_to_n1000": current_exponent,
            "empirical_upper_bound": exponent_upper_bound,
            "limit": 1.3,
        },
        "last_two_rung_per_base_slopes": {
            "n500": prior_slopes,
            "n1000": current_slopes,
            "relative_changes": slope_changes,
            "absolute_change_limit": 0.25,
        },
        "rung_observations": all_observations,
        "final_scaling_models": {
            "method": "power-law fit over frozen N=10/100/250/500/1000 observations; descriptive at authorized ceiling only",
            "fits_at_n1000_incremental_bases": models,
        },
        "pinned_n1000_projection": predecessor_projection,
        "checks": checks,
    }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(projected) != COHORT_ROWS or projected != set(inputs["accessions"]) or not projected.issubset(allowed):
        raise GateError("projected global union is not exactly frozen N=1,000")
    ...
    shutil.copyfile(repo / "manifests/pilot-cohorts-v1/cohort-1000.tsv", stage / "manifests/cohort-1000.tsv")
    ...
    provenance = {
        "schema": "canonical-cohort-1000-provenance-v1", "task_id": TASK_ID, "release_id": rid,
        ...
        "predecessor_canonical_scale_trend": "PASS",
        "predecessor_scale_trend_sha256": PREDECESSOR_SCALE_TREND_SHA256,
        "predecessor_n1000_projection": "PASS",
        ...
        "integrated_gate_rationale": "canonical-only acquisition consumes the pinned N=500 canonical scale trend and N=1,000 projection; extraction/query remains blocked and no integrated GO is claimed",
        "reuse_policy": "rows 1-500 are read-only predecessor digest references; no copy, download, or recompression",
    ...
    try:
        ...
        scale_path = stage / "scale_trend.json"
        failed_rows = _jsonl(failures)
        preserved_scale = None
        if scale_path.is_file() and any(
            row.get("event") == "RUN_FAILED" and "system_swap_growth_zero" in row.get("message", "")
            for row in failed_rows
        ):
            candidate = json.loads(scale_path.read_text())
            observation = candidate.get("rung_observations", [{}])[-1]
            current_new = assemblies[PREDECESSOR_ROWS:]
            expected_identity = {
                "rung": COHORT_ROWS,
                "incremental_objects": NEW_ROWS,
                "incremental_bases": sum(int(row["total_bases"]) for row in current_new),
                "source_bytes": sum(int(row["source_package_bytes"]) for row in current_new),
                "release_id": release_id(inputs["accessions"]),
            }
            if (candidate.get("verdict") != "PASS" or not all(candidate.get("checks", {}).values())
                    or any(observation.get(key) != value for key, value in expected_identity.items())):
                raise GateError("prior failed-at-promotion scale observation is not independently reusable")
            recomputed = compute_scale_trend(
                repo, inputs, assemblies, float(observation["wall_seconds"]),
                int(observation["peak_rss_bytes"]), int(observation["stage_bytes"]),
                int(observation["stage_files"]), allocations, args.n1000_time_allocation_seconds,
            )
            if recomputed != candidate:
                raise GateError("prior failed-at-promotion scale observation recomputation mismatch")
            preserved_scale = candidate
        scale_trend = preserved_scale or compute_scale_trend(
        ...
        scale_observation_row = scale_trend["rung_observations"][-1]
        scale_observation = {
            "source": "CHECKSUM_VALIDATED_PRIOR_ATTEMPT" if preserved_scale else "CURRENT_INVOCATION",
            "wall_seconds": scale_observation_row["wall_seconds"],
            "peak_rss_bytes": scale_observation_row["peak_rss_bytes"],
            "stage_bytes": scale_observation_row["stage_bytes"],
            "stage_files": scale_observation_row["stage_files"],
        }
        certified_peak_rss = max(peak_rss, int(scale_observation["peak_rss_bytes"]))
        certified_stage_bytes = max(max_usage[0], int(scale_observation["stage_bytes"]))
        certified_stage_files = max(max_usage[1], int(scale_observation["stage_files"]))
        scale_path.write_bytes(canonical_json(scale_trend))
        resource_checks = {
            "all_preflight_records_pass": all(json.loads(line).get("verdict") == "PASS" for line in resources.read_text().splitlines()),
            "peak_rss_le_70pct_assigned": certified_peak_rss * 100 <= allocations.assigned_ram_bytes * 70,
            "process_swap_events_zero": process_swaps == 0,
            "system_swap_growth_zero": system_swap_growth == 0,
            "measured_peak_bytes_le_70pct_allocation": certified_stage_bytes * 100 <= allocations.durable_allocation_bytes * 70,
            "measured_files_le_50pct_inodes": certified_stage_files * 2 <= allocations.inode_allocation,
            "configured_projection_ge_modeled_upper95": allocations.predicted_durable_peak_bytes >= modeled_upper95_peak,
            "measured_peak_le_modeled_upper95": certified_stage_bytes <= modeled_upper95_peak,
        ...
        resource_summary = {
            "schema": "canonical-cohort-1000-resource-summary-v1", "verdict": "PASS",
            "allocations": asdict(allocations), "peak_rss_bytes": certified_peak_rss,
            "peak_rss_fraction": certified_peak_rss / allocations.assigned_ram_bytes,
            "process_swap_events": process_swaps, "system_swap_growth_bytes": system_swap_growth,
            "measured_release_stage_peak_bytes": certified_stage_bytes, "measured_release_stage_peak_files": certified_stage_files,
            ...
            "disk_projection": {
                "method": "N=500 measured stage peak linearly scaled by new objects plus conservative 25% upper-95 allowance",
                "predecessor_release_id": PREDECESSOR_RELEASE_ID,
                ...
                "observed_to_linear_ratio": certified_stage_bytes / linear_peak_projection,
            },
            "scale_trend": scale_trend,
            "consumed_n1000_upper95_projection": scale_trend["pinned_n1000_projection"],
        }
        (stage / "resource_summary.json").write_bytes(canonical_json(resource_summary))
        state_text = state.read_text()
        restart = {
            "schema": "canonical-cohort-1000-restart-evidence-v1",
        ...
        gates = {
            ...
            "canonical_n500_scale_trend": "PASS", "canonical_n1000_projection": "PASS", "canonical_scale_trend": "PASS",
        ...
        release = {
            ...
            "input_cohort_1000_sha256": COHORT_SHA256, "predecessor_release_id": PREDECESSOR_RELEASE_ID,
            "predecessor_release_json_sha256": PREDECESSOR_RELEASE_JSON_SHA256,
            "predecessor_scale_trend_verdict": "PASS",
            "predecessor_scale_trend_sha256": PREDECESSOR_SCALE_TREND_SHA256,
            "predecessor_n1000_projection_verdict": "PASS",
            "compatibility_release_id": COMPATIBILITY_RELEASE_ID,
            "compatibility_release_json_sha256": COMPATIBILITY_RELEASE_JSON_SHA256,
            ...
            "object_storage_contract": "rows 1-500 resolve read-only through the N=500 predecessor's immutable object references and inventory SHA; rows 501-1000 are self-contained",
        ...
        manifest_specs = {
            "cohort-1000.tsv": COHORT_ROWS, "assemblies.tsv": COHORT_ROWS, "contigs.tsv": len(contigs),
        ...
        if global_finish["distinct_exact_assembly_revisions"] != COHORT_ROWS or set(global_finish["accessions"]) != set(inputs["accessions"]):
            raise GateError("finish global union is not exactly frozen N=1,000")
        publish_tracked(final, tracked, artifact)
        return release
...
def parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tracked-root", default="manifests/canonical-cohort-1000-v1")
    parser.add_argument("--artifact-root", default="artifacts/canonical_cohort_1000")
...
def main() -> int:
    try:
        ...
        if args.inject_accession not in inputs["accessions"][PREDECESSOR_ROWS:]:
            raise GateError("kill injection must target a new frozen cohort row 501..1000")
