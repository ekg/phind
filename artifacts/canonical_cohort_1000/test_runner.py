...
class CanonicalCohort1000Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_pinned_real_inputs_are_exact_nested_1000_and_all_required_gates_pass(self):
        ...
        predecessor_root, predecessor_id, _, _ = r._storage_for(500, inputs["accessions"][499], Path("stage"), inputs)
        self.assertNotEqual(first_id, "SELF")
        self.assertEqual(predecessor_id, r.PREDECESSOR_RELEASE_ID)
        self.assertNotEqual(first_root, predecessor_root)
        self.assertEqual(inputs["canonical_scale_trend"]["verdict"], "PASS")
        self.assertEqual(inputs["canonical_scale_trend"]["n1000_projection"]["target_rung"], 1000)
        self.assertTrue(all(inputs["canonical_scale_trend"]["n1000_projection"]["checks"].values()))

    def test_manifest_row_checksum_mismatch_fails_closed(self):
        original = Path("manifests/pilot-cohorts-v1/cohort-1000.tsv").read_text()
    ...
    def test_url_constructor_is_bounded_to_exact_frozen_1000(self):
        inputs = r.verify_inputs(Path(".").resolve())
        r.configure_pinned_primitives(inputs["accessions"])
        self.assertIn(inputs["accessions"][999], p.download_url(inputs["accessions"][999]))
        with self.assertRaisesRegex(p.GateError, "outside immutable"):
            p.download_url("GCF_999999999.1")
        with self.assertRaisesRegex(p.GateError, "outside immutable"):
            p.download_url(inputs["accessions"][250].rsplit(".", 1)[0])
    ...
    def test_canonical_scale_projection_checksum_inventory_mismatch_fails_closed(self):
        predecessor = {
            "release_id": r.PREDECESSOR_RELEASE_ID,
            "applicable_gates": {"canonical_scale_trend": "PASS"},
            "scale_trend_sha256": r.PREDECESSOR_SCALE_TREND_SHA256,
        }
        with mock.patch.object(r, "sha_file", return_value="0" * 64):
            with self.assertRaisesRegex(p.GateError, "artifact inventory checksum mismatch"):
                r._verify_canonical_scale_projection(Path("."), Path("unused"), predecessor)

    def test_scale_thresholds_refuse_exponent_or_slope_regression(self):
        with self.assertRaisesRegex(p.GateError, "scale trend NO_GO"):
            r._enforce_scale_thresholds(1.300001, {"wall_seconds_per_new_base": 0.0})
        with self.assertRaisesRegex(p.GateError, "scale trend NO_GO"):
            r._enforce_scale_thresholds(1.0, {"wall_seconds_per_new_base": 0.250001})
        checks = r._enforce_scale_thresholds(1.3, {"wall_seconds_per_new_base": -0.25})
        self.assertTrue(all(checks.values()))

    def test_conditional_or_failed_applicable_gate_is_never_waived(self):
        for verdict in ("CONDITIONAL", "NO_GO", "FAIL", ""):
            with self.subTest(verdict=verdict), self.assertRaisesRegex(p.GateError, "not unqualified PASS/NA"):
                r._require_pass_or_na({"canonical_scale_trend": verdict}, "test")

    def test_global_union_of_1001_exact_revisions_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            release_root = root / "releases" / "other" / "release"
            release_root.mkdir(parents=True)
            accessions = [f"GCF_{number:09d}.1" for number in range(1, 1002)]
            (release_root / "COMPLETE").write_text("marker")
            (release_root / "release.json").write_text(json.dumps({
                "counts": {"distinct_sequence_bearing_assemblies": 1001},
                "sequence_bearing_assembly_accessions": accessions,
            }))
            with self.assertRaisesRegex(p.GateError, "global distinct assembly cap"):
                r.audit_global_release_cap(root, set(accessions))
    ...
    def test_release_identity_is_order_and_pin_deterministic(self):
        ...
        self.assertTrue(first.startswith("canonical-cohort-1000-v1-"))
